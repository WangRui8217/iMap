...
def extract_features(design_file, yosys_binary='yosys', imap_binary='yosys-abc') -> Dict[str, float]:
    ...
    try:
        ...
        stats = {}
        thread = Thread(target=aig_stats, args=(design_file, imap_binary, stats))
...
class StateDesign:
    ...
    def __init__(self, n_inputs: float, pis: float, pos: float, area: float, depth: float,
