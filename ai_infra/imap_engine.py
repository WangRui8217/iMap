...
class EngineIMAP():
    ...
    def get_aig_stats(self)->Dict[str, Union[int,int]]:
        """ Get the profile of current AIG-network

        Returns:
            dict: A dictionary of the profile of current AIG-network
        """
        command='read_aiger -f' + self.input_file +'; print_stats -t 0'
        proc=check_output([imap,'-c',command])
        ...
        return results
    def get_FPGA_stats(self)->Dict[str, Union[int,int]]:
        """ Get the profile of current AIG-network

        Returns:
            dict: A dictionary of the profile of current AIG-network
        """
        command='read_aiger -f' + self.input_file +'; print_stats -t 1'
        proc=check_output([imap,'-c',command])
        lines = proc.decode("utf-8").split('\n')
        for line in lines:
            if 'Stats of FPGA' in line:
                results = {}
                ob = re.search(r'pis *= *[0-9]+', line)
                results['pis'] = int(ob.group().split('=')[1].strip())
                ob = re.search(r'pos *= *[0-9]+', line)
